...
exit /b 1
:run
echo 将向 ZCode 输入框测试发送一次续命消息:
echo   「任务意外中断,请继续!」
echo 10 秒后发送,期间请把 ZCode 打开并停在所要测试的对话。
echo 按 Ctrl+C 可取消本次测试。
echo.
if "%PYCT%"=="py" (
  py -3 zcode_resume_watcher.py --test-send
) else (
  "%PYCT%" zcode_resume_watcher.py --test-send
)
pause
